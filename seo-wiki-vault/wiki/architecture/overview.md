...
- **Dr Jasmine** (`apps/dr-jasmine`) — single-home native site (`/` + `/blog` + `/admin`); no `/about` `/workshop` `/programs` `/faq` pages. GHL capture is archive only: `raw/research/dr-jasmine-ghl-capture/`.
