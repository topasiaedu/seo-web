...
- **CAE** is the first brand app (`apps/cae`) — **native ZWDS** homepage (Insights Blog soft bento; home SSR) + native `/media/`; **Admin** at `/cae/admin` and **public blog** at `/cae/blog` (server mode). Historical GHL scrapes stay in `seo-wiki-vault/raw/research/`.
