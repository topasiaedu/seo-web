...
- **Alt/title** are first-class media metadata (`media` table + CMS Media Library). Interim landing alt work originally landed on the GHL dump; **live CAE marketing pages** use local `apps/cae/src/assets/` (+ `assets/media/`) remapped from capture CDN URLs — see [cae-ghl-section-lift-and-media-page](cae-ghl-section-lift-and-media-page.md). Meaningful alts / loading / single-h1 are now applied by the **SEO remapper pass** — see [cae-seo-improvements](cae-seo-improvements.md).
...
- [sites/cae.md](../sites/cae.md) — GHL lift + local assets + remapper SEO; Storage rewire still deferred
...
- [sources/cae-seo-improvements.md](cae-seo-improvements.md) — interim alt / head / sitemap implementation
