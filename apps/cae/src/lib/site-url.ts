/**
 * @fileoverview Absolute URL helpers for CAE SEO (canonical, Open Graph, JSON-LD).
 */

/** Default production origin when `PUBLIC_SITE_ORIGIN` is unset. */
export const DEFAULT_SITE_ORIGIN = "https://caegoh.com";
 ...
 *
 * @returns Origin such as `https://caegoh.com`
 */
 ...
 *
 * @param pathOrUrl - Path (`/cae/...`), Astro asset src, or already-absolute URL
 ...
 *
 * @param pathname - `Astro.url.pathname` (includes base when under `/cae/`)
...
export function toCanonicalUrl(pathname: string): string {
  ...
  // Prefer trailing slash for index-style marketing routes.
