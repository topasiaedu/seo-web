/**
 * @fileoverview JSON-LD helpers for Dr Jasmine public blog index and Post pages.
 */
...
const DEFAULT_SITE_NAME = "Dr Jasmine";

/** Default byline when a Post has no joined Author. */
const DEFAULT_AUTHOR_NAME = "Dr Jasmine";
...
export type BlogIndexJsonLdInput = {
  ...
  pathname: string;
  /** Collection / Blog display name. */
  name: string;
  /** Meta description for the index CollectionPage. */
  description: string;
  /** Brand name for Organization / WebSite. Defaults to `"Dr Jasmine"`. */
  siteName?: string;
...
export type BlogPostJsonLdInput = {
  ...
  pathname: string;
  /** Brand name for publisher / WebSite. Defaults to `"Dr Jasmine"`. */
  siteName?: string;
  /** Absolute image URL (OG or hero). */
  ogImageUrl?: string;
  /** Absolute URL of the blog index. Defaults to `{origin}{base}blog/`. */
  blogIndexUrl?: string;
  /** Headline override (e.g. resolved `seoTitle`). */
  headline?: string;
  /** Description override (SEO description / excerpt). */
  description?: string;
};
 ...
 *
 * @param node - JSON-LD document root
 ...
 */
function readingTimeDuration(minutes: number | null): string | undefined {
