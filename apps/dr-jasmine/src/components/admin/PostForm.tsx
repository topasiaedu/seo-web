/**
 * @fileoverview Dr Jasmine Admin Post create / edit form (React island).
 *
 * Wires TipTap BodyEditor plus FAQ / sources / tags widgets.
 * Persists via `@seo/blog` with `site_id` always from props (DJ project id).
 * Cover images upload to Supabase Storage `media/dr-jasmine/blog/covers/` with URL paste fallback.
...
export type PostFormProps = {
  ...
  mode: "create" | "edit";
  /** Dr Jasmine brand `sites.id` UUID (`drJasmineSiteConfig.projectId`). */
  siteId: string;
  /** Absolute-from-origin path to the Posts list (e.g. `/dr-jasmine/admin/posts`). */
  postsListHref: string;
  /**
   * Absolute-from-origin base for post edit URLs without trailing slash
   * (e.g. `/dr-jasmine/admin/posts`). Create redirects to `{postsBaseHref}/{id}/edit`.
   */
...
/**
 * Full Post create / edit form for Dr Jasmine Admin.
 *
      ...
      <header className={styles.header}>
        <div>
          ...
          <p className={styles.lede}>
            Create and manage a Dr Jasmine Post. Slug and reading time are generated
